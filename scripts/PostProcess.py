...
def PostProcess(inputfile, outputfile, outputimginitial, outputdeformed):
	content = []
	...
	for i in range(0, nodesCount):
		...
		line+=1
		nodes.append( ( float(coordinates[0]), float(coordinates[1]) ) )
	...
	for i in range(0, elementsCount):
		...
		line+=1
		elements.append( ( int(elementNodes[0]), int(elementNodes[1]), int(elementNodes[2]) ) )
    ...
    # Почему это не работает??? Это же верно ...
	#deforms = [ float(line) for line in resultsContent[nodesCount*2:] ]
	#stresses = [ float(line) for line in resultsContent[:nodesCount*2] ]
	...
	image.save(outputimginitial, "PNG")
...
if __name__ == '__main__':
	...
	parser.add_argument('outputimginitial', metavar='outputimginitial', type=str, help='Path to output file.')
	parser.add_argument('outputdeformed', metavar='outputdeformed', type=str, help='Path to output file.')
	args = parser.parse_args()
	PostProcess(args.inputfile, args.outputfile, args.outputimginitial, args.outputdeformed)
